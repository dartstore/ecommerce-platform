 ...
 *      fail repeatedly, so a downstream problem must not surface as a
 *      5xx to the provider.
 *
...
export class WebhookIngestionService {
  ...
  async ingest(input: {
    ...
    if (!input.rawBody || input.rawBody.length === 0) {
      ...
      return { outcome: 'unsupported', factCount: 0, detail: 'missing raw body' }
    }
    ...
    try {
      ...
      return { outcome: 'unmatched', factCount: 0, detail: 'bad account id' }
    }

    const account = await this.prisma.paymentAccount.findFirst({
      where: { id: accountId },
      select: { id: true, store_id: true, mode: true, gateway: true, status: true },
    })

    if (!account) {
      this.logger.warn(`Webhook for unknown account ${input.accountId}.`)
      return { outcome: 'unmatched', factCount: 0, detail: 'unknown account' }
    }

    if (!this.providers.has(account.gateway)) {
      return { outcome: 'unsupported', factCount: 0, detail: 'no adapter' }
    }
    ...
    if (signingSecret.length === 0) {
      ...
      return { outcome: 'unsupported', factCount: 0, detail: 'no signing secret' }
    }
    ...
    try {
      ...
      if (error instanceof ProviderError) {
        ...
        return { outcome: 'unsupported', factCount: 0, detail: error.code }
      }
      throw error
    }
    ...
    if (facts.length === 0) {
      ...
      return { outcome: 'ignored', factCount: 0 }
    }
    ...
    if (applied > 0) {
      return { outcome: 'applied', factCount: applied }
    }
    ...
    if (unmatched === results.length) {
      ...
      return { outcome: 'unmatched', factCount: results.length }
    }

    return { outcome: 'duplicate', factCount: results.length }
  }
}
