...
export class LedgerService {
    ...
    try {
      ...
      await tx.ledgerPosting.createMany({
        data: input.postings.map((posting, index) => ({
          entry_id: entry.id,
          ledger_account_id: accountIds[index],
      ...
      if (isUniqueConstraintError(error)) {
        const raced = await tx.journalEntry.findFirstOrThrow({
          where: {
            dedupe_key: input.dedupeKey,
            store_id: input.storeId,
            mode: input.mode,
  ...
  async reverse(
    ...
    const reversal = await tx.journalEntry.create({
      data: {
        ...
        entry_type: `${original.entry_type}.reversal`,
  ...
  async summary(storeId: bigint, mode: Mode): Promise<LedgerBalance[]> {
    /**
     * This raw query is explicitly tenant-scoped by store_id + mode.
     * It is intentionally kept raw because the aggregate spans ledger
     * postings through ledger_accounts.
     */
  ...
  /**
   * Platform-wide integrity check.
   *
   * ledger_postings intentionally has no store_id of its own; it is scoped
   * through JournalEntry/LedgerAccount. This health check therefore scans
   * all entries and must use the explicit health_check cross-store reason.
   */
  async findUnbalancedEntries(limit = 50): Promise<bigint[]> {
    const rows = await crossStoreQuery(
      'health_check',
      'find unbalanced ledger entries',
      () =>
        this.prisma.$queryRaw<EntryIdRow[]>`
          SELECT entry_id
          FROM ledger_postings
          GROUP BY entry_id
          HAVING SUM(
  ...
  private async resolveAccounts(
    ...
    for (const posting of input.postings) {
      ids.push(await this.resolveAccount(tx, input, posting));
    }
