...
export class OrderService {
  ...
  private async generateOrderNumber(storeId: bigint): Promise<string> {
    const count = await this.prisma.guarded().order.count({
  ...
  async createOrder(
    ...
    const order = await this.prisma.guarded().$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (variant.track_inventory) {
          await tx.productVariant.update({
            ...
            data: {
              inventory_qty: {
                decrement: Math.max(1, Math.floor(item.qty)),
              },
            },
          });
        }
  ...
  async getOrders(
    ...
    const [total, orders] = await Promise.all([
      this.prisma.guarded().order.count({ where }),
      this.prisma.guarded().order.findMany({
        where,
        include: { items: true },
  ...
  async updateOrderStatus(
    ...
    return this.jsonSafe(
      await this.prisma.guarded().order.update({
        where: {
          id: order.id,
          store_id: storeId,
        },
