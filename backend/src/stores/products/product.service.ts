...
export class ProductService {
  ...
  async getStore(userId: any, storeSlug?: string) {
  ...
  async updateProductStatus(userId: any, productId: string, status: ProductStatus) {
    const store = await this.getStore(userId)
    const product = await this.prisma.product.findFirst({
      where: { id: BigInt(productId), store_id: store.id },
    })
  ...
  async duplicateProduct(userId: any, productId: string) {
    const store = await this.getStore(userId)
    const product = await this.prisma.product.findFirst({
      where: { id: BigInt(productId), store_id: store.id },
      include: {
  ...
  async addProductImages(userId: any, productId: string, images: { url: string; alt?: string; key?: string }[]) {
    const store = await this.getStore(userId)
    const product = await this.prisma.product.findFirst({
      where: { id: BigInt(productId), store_id: store.id },
    })
  ...
  async createProductType(userId: any, name: string) {
    ...
    const exists = await this.prisma.productType.findFirst({
      where: { store_id: store.id, name: { equals: name.trim(), mode: 'insensitive' } },
    })
    if (exists) return this.jsonSafe(exists)
    return this.jsonSafe(
      await this.prisma.productType.create({ data: { store_id: store.id, name: name.trim() } }),
    )
  }
  ...
  async updateProductType(userId: any, id: string, name: string) {
    ...
    const type = await this.prisma.productType.findFirst({
      where: { id: BigInt(id), store_id: store.id },
    })
  ...
  async createTag(userId: any, name: string) {
    ...
    if (exists) return this.jsonSafe(exists)
    return this.jsonSafe(
      await this.prisma.tag.create({ data: { store_id: store.id, name: name.trim() } }),
    )
  }
