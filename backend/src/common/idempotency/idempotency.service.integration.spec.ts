...
describe('IdempotencyService (integration)', () => {
  ...
  it('replays the stored response for an identical repeat', async () => {
    ...
    await service.complete(first.recordId, STORE, MODE, 201, { id: 'pi_123' });
  ...
  it('stores an empty response body without throwing', async () => {
    ...
    await expect(
      service.complete(first.recordId, STORE, MODE, 204, undefined),
  ...
  it('returns conflict for the same key with a different body', async () => {
    ...
    await service.complete(first.recordId, STORE, MODE, 200, { ok: true });
  ...
  it('allows exactly one winner under real parallel claims', async () => {
    ...
    try {
      ...
      expect(
        results.filter((result) => result.outcome === 'proceed'),
      ).toHaveLength(1);

      expect(
        results.filter((result) => result.outcome === 'in_flight'),
      ).toHaveLength(3);
    } finally {
  ...
  it('clears a stale response body when a lease is stolen', async () => {
    ...
    await service.fail(first.recordId, STORE, MODE);
  ...
  it('isolates identical keys across modes', async () => {
    const live = await service.claim(
      baseRequest({
        mode: 'live',
      }),
    );

    const test = await service.claim(
      baseRequest({
        mode: 'test',
      }),
    );
  ...
  it("does not complete another store's record", async () => {
    ...
    await service.complete(first.recordId, OTHER_STORE, MODE, 200, {
      leaked: true,
    });
