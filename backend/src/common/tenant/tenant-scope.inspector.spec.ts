...
describe('inspectScope', () => {
  ...
  it('flags deletes and updates without scope', () => {
    for (const op of ['updateMany', 'deleteMany', 'update', 'delete']) {
      expect(
        inspectScope({ model: M, operation: op, args: { where: { id: 1n } } })
          .length,
      ).toBeGreaterThan(0);
    }
  });
...
describe('relation-nested store scope', () => {
  ...
  it('flags Collection reads without storeId scope', () => {
    const violations = inspectScope({
      model: 'Collection',
      operation: 'findMany',
      args: {
    ...
    expect(violations.some((v) => v.kind === 'missing_store_scope')).toBe(true);
  });

  it('accepts Collection reads scoped by storeId', () => {
    const violations = inspectScope({
      model: 'Collection',
      operation: 'findMany',
      args: {
