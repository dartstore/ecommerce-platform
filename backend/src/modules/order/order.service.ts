import { Injectable, NotFoundException, BadRequestException } from '@nestjs/common'
import { PrismaService } from '../../prisma/prisma.service'
import { OrderStatus, ProductStatus } from '@prisma/client'
import { PaymentCheckoutService } from '../payments/providers/payment-checkout.service'
import { getProviderDef } from '../payments/payment-providers.registry'
import { GetOrdersQueryDto } from './dto/get-orders-query.dto'

interface CheckoutItemInput {
  variantId: string
  qty: number
}

interface CheckoutCustomerInput {
  name: string
  phone: string
  email?: string
  address: string
  city: string
  notes?: string
}

const MAX_ORDER_NUMBER_RETRIES = 5

@Injectable()
export class OrderService {
  constructor(
    private prisma: PrismaService,
    private paymentCheckout: PaymentCheckoutService,
  ) {}

  private jsonSafe(data: any) {
    return JSON.parse(JSON.stringify(data, (_, v) =>
      typeof v === 'bigint' ? v.toString() : v
    ))
  }

  private async getStore(userId: any) {
    const store = await this.prisma.store.findFirst({ where: { ownerId: BigInt(userId) } })
    if (!store) throw new NotFoundException('Store not found')
    return store
  }

  /** رقم طلب بسيط وقابل للقراءة: #1001, #1002... لكل متجر لوحده */
  private async generateOrderNumber(storeId: bigint): Promise<string> {
    const count = await this.prisma.order.count({ where: { store_id: storeId } })
    return String(1000 + count + 1)
  }

  async createOrder(
    storeSlug: string,
    customer: CheckoutCustomerInput,
    items: CheckoutItemInput[],
    paymentMethod: string,
  ) {
    if (!items || items.length === 0) throw new BadRequestException('السلة فاضية')
    if (!customer?.name || !customer?.phone || !customer?.address || !customer?.city) {
      throw new BadRequestException('بيانات العميل ناقصة')
    }
    if (!paymentMethod) throw new BadRequestException('اختر طريقة دفع')

    const providerDef = getProviderDef(paymentMethod)
    if (!providerDef) throw new BadRequestException('طريقة دفع غير معروفة')

    const store = await this.prisma.store.findFirst({ where: { slug: storeSlug } })
    if (!store) throw new NotFoundException('Store not found')

    // ✅ تحقق حقيقي إن البوابة دي فعلاً مفعّلة للمتجر ده — مش مجرد ثقة في الفرونت
    const providerRecord = await this.prisma.storePaymentProvider.findUnique({
      where: { store_id_provider: { store_id: store.id, provider: paymentMethod as any } },
    })
    if (!providerRecord?.enabled) {
      throw new BadRequestException(`${providerDef.name_ar} غير مفعّلة في هذا المتجر`)
    }

    const variantIds = items.map((i) => BigInt(i.variantId))
    const variants = await this.prisma.productVariant.findMany({
      where: { id: { in: variantIds } },
      include: { product: true },
    })

    const orderItemsData: any[] = []
    let subtotal = 0

    for (const item of items) {
      const variant = variants.find((v) => v.id === BigInt(item.variantId))
      if (!variant) {
        throw new BadRequestException(`منتج غير موجود (variant ${item.variantId})`)
      }
      if (variant.product.store_id !== store.id) {
        throw new BadRequestException('منتج لا ينتمي لهذا المتجر')
      }
      if (
        variant.product.status !== ProductStatus.ACTIVE &&
        variant.product.status !== ProductStatus.UNLISTED
      ) {
        throw new BadRequestException(`المنتج "${variant.product.title}" غير متاح حالياً`)
      }
      const qty = Math.max(1, Math.floor(item.qty))
      if (variant.track_inventory && !variant.continue_selling && variant.inventory_qty < qty) {
        throw new BadRequestException(`الكمية المطلوبة من "${variant.product.title}" غير متوفرة`)
      }

      const price = Number(variant.price)
      subtotal += price * qty

      orderItemsData.push({
        product_id: variant.product_id,
        variant_id: variant.id,
        title: variant.product.title,
        variant_title: variant.title === 'Default Title' ? null : variant.title,
        price,
        qty,
        image_url: variant.image_url || null,
      })
    }

    const total = subtotal
    const isOnline = this.paymentCheckout.isOnlineProvider(paymentMethod)

    const order = await this.createOrderWithRetry({
      store,
      customer,
      orderItemsData,
      paymentMethod,
      subtotal,
      total,
      items,
      variants,
      isOnline,
    })

    if (!isOnline) {
      return this.jsonSafe({ order, payment_redirect_url: null })
    }

    // 💳 بوابة أونلاين — لازم نجيب رابط دفع حقيقي دلوقتي
    try {
      const result = await this.paymentCheckout.charge(paymentMethod, {
        storeId: store.id,
        storeSlug: store.slug,
        orderNumber: order.order_number,
        amount: total,
        currency: store.currency,
        customer: {
          name: customer.name, phone: customer.phone,
          email: customer.email, address: customer.address, city: customer.city,
        },
        items: orderItemsData.map((i) => ({ title: i.title, price: i.price, qty: i.qty })),
      })
      return this.jsonSafe({ order, payment_redirect_url: result.redirectUrl })
    } catch (err) {
      // فشل الاتصال بالبوابة — نلغي الطلب عشان المخزون يفضل سليم
      await this.prisma.order.update({ where: { id: order.id }, data: { status: OrderStatus.CANCELLED } })
      throw err
    }
  }

  /**
   * بينشئ الطلب داخل transaction، وينقص المخزون بشكل atomic (شرط ضمن نفس الـ update
   * مش فحص منفصل قبله) عشان يمنع الـ oversell لو طلبين وصلوا في نفس اللحظة.
   * لو رقم الطلب اتصادم (unique constraint) بسبب سباق بين طلبين، بيعيد المحاولة
   * برقم جديد لحد MAX_ORDER_NUMBER_RETRIES مرة.
   */
  private async createOrderWithRetry(params: {
    store: { id: bigint; slug: string; currency: string }
    customer: CheckoutCustomerInput
    orderItemsData: any[]
    paymentMethod: string
    subtotal: number
    total: number
    items: CheckoutItemInput[]
    variants: any[]
    isOnline: boolean
    attempt?: number
  }): Promise<any> {
    const {
      store, customer, orderItemsData, paymentMethod,
      subtotal, total, items, variants, isOnline, attempt = 0,
    } = params

    const orderNumber = await this.generateOrderNumber(store.id)

    try {
      return await this.prisma.$transaction(async (tx) => {
        const created = await tx.order.create({
          data: {
            store_id: store.id,
            order_number: orderNumber,
            status: OrderStatus.PENDING,
            payment_method: paymentMethod,
            customer_name: customer.name.trim(),
            customer_phone: customer.phone.trim(),
            customer_email: customer.email?.trim() || null,
            address_line: customer.address.trim(),
            city: customer.city.trim(),
            notes: customer.notes?.trim() || null,
            subtotal,
            total,
            items: { create: orderItemsData },
          },
          include: { items: true },
        })

        // ⚠️ المخزون بينقص فوراً بس للبوابات اليدوية (COD / تحويل بنكي).
        // للبوابات الأونلاين، بننقصه بس لما الدفع ينجح فعلياً في الـ webhook.
        if (!isOnline) {
          for (const item of items) {
            const variant = variants.find((v) => v.id === BigInt(item.variantId))!
            if (!variant.track_inventory) continue

            const qty = Math.max(1, Math.floor(item.qty))

            // الشرط هنا جزء من نفس عملية الـ update (atomic) — مفيش فحص منفصل
            // قبلها ممكن يبقى قديم لحظة ما التنفيذ يوصله (race condition).
            const result = await tx.productVariant.updateMany({
              where: {
                id: variant.id,
                ...(variant.continue_selling ? {} : { inventory_qty: { gte: qty } }),
              },
              data: { inventory_qty: { decrement: qty } },
            })

            if (result.count === 0) {
              throw new BadRequestException(`الكمية المطلوبة من "${variant.product.title}" غير متوفرة`)
            }
          }
        }

        return created
      })
    } catch (err: any) {
      // P2002 = unique constraint violation (store_id, order_number) بسبب سباق بين طلبين
      if (err.code === 'P2002' && attempt < MAX_ORDER_NUMBER_RETRIES) {
        return this.createOrderWithRetry({ ...params, attempt: attempt + 1 })
      }
      throw err
    }
  }

  /** جلب طلب واحد بالـ order number — مستخدمة في صفحة تأكيد الطلب public */
  async getStorefrontOrder(storeSlug: string, orderNumber: string) {
    const store = await this.prisma.store.findFirst({ where: { slug: storeSlug } })
    if (!store) throw new NotFoundException('Store not found')

    const order = await this.prisma.order.findFirst({
      where: { store_id: store.id, order_number: orderNumber },
      include: { items: true },
    })
    if (!order) throw new NotFoundException('Order not found')
    return this.jsonSafe(order)
  }

  // ── لوحة تحكم التاجر ────────────────────────────────────────────────────

  async getOrders(userId: any, filters: GetOrdersQueryDto) {
    const store = await this.getStore(userId)
    const where: any = { store_id: store.id }
    if (filters.status) where.status = filters.status
    if (filters.search) {
      where.OR = [
        { order_number: { contains: filters.search, mode: 'insensitive' } },
        { customer_name: { contains: filters.search, mode: 'insensitive' } },
        { customer_phone: { contains: filters.search, mode: 'insensitive' } },
      ]
    }
    const [total, orders] = await Promise.all([
      this.prisma.order.count({ where }),
      this.prisma.order.findMany({
        where,
        include: { items: true },
        orderBy: { created_at: 'desc' },
        skip: (filters.page - 1) * filters.limit,
        take: filters.limit,
      }),
    ])
    return this.jsonSafe({
      orders,
      total,
      page: filters.page,
      pages: Math.ceil(total / filters.limit),
    })
  }

  async getOrder(userId: any, orderId: string) {
    const store = await this.getStore(userId)
    const order = await this.prisma.order.findFirst({
      where: { id: BigInt(orderId), store_id: store.id },
      include: { items: true },
    })
    if (!order) throw new NotFoundException('Order not found')
    return this.jsonSafe(order)
  }

  async updateOrderStatus(userId: any, orderId: string, status: OrderStatus) {
    const store = await this.getStore(userId)
    const order = await this.prisma.order.findFirst({
      where: { id: BigInt(orderId), store_id: store.id },
    })
    if (!order) throw new NotFoundException('Order not found')
    return this.jsonSafe(
      await this.prisma.order.update({ where: { id: order.id }, data: { status } }),
    )
  }
}
