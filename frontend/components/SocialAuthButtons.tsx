...
const PROVIDERS = [
  ...
  },
  {
    ...
    bg: 'bg-white hover:bg-gray-50 border border-gray-200 text-gray-700',
    icon: <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#F35325" d="M1 1h10v10H1z"/><path fill="#81BC06" d="M13 1h10v10H13z"/><path fill="#05A6F0" d="M1 13h10v10H1z"/><path fill="#FFBA08" d="M13 13h10v10H13z"/></svg>,
  },
] as const
...
export default function SocialAuthButtons({ mode, accountType = 'individual', className = '' }: Props) {
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-3 bg-white dark:bg-gray-900 text-gray-500">
            {mode === 'login' ? 'أو سجّل دخولك عبر' : 'أو أنشئ حسابك عبر'}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {PROVIDERS.map((p) => (
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium text-sm transition-all duration-200 shadow-sm hover:shadow-md active:scale-95 ${p.bg}`}
          >
            {p.icon}
            <span>{p.label}</span>
          </button>
